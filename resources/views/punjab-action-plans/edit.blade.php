...
@push('title', 'Update Punjab Action Plan')
                ...
                <div class="">
                    ...
                    <div class="card card-custom gutter-b">
                        <div class="card-header">
                            <h3 class="card-title">Update Punjab Action Plan</h3>

                        </div>

                        <!--begin::Form-->
                        {!! Form::open(array('id'=>'form_add_edit','route' => ['punjab-action-plans.update', $punjabActionPlan],'method'=>'PUT','files' => 'true')) !!}
                        <div class="card-body">
                                ...
                                <div class="col-lg-6 @if(!empty(auth()->user()->department_id)) d-none @endif">
                                    ...
                                    <select class="form-control select2" name="department_id">
                                        ...
                                        @foreach($departments as $department)
                                            <option {{ old('department_id',$punjabActionPlan->department_id)== $department->id ? 'selected': '' }} value="{{ $department->id }}"> {{ $department->department_name }} </option>
                                        @endforeach
                                    </select>
                                    @error('department_id')
                                ...
                                <div class="col-lg-6">
                                    ...
                                    <select class="form-control select2" name="year">
                                        ...
                                        @for($i=date('Y'); $i>=2015; $i--)
                                            <option {{ old('year', $punjabActionPlan->year)== $i ? 'selected': '' }} value="{{ $i }}"> {{ $i }} </option>
                                        @endfor
                                    </select>
                                    @error('year')
                                    <div class="error">{{ $message }}</div>
                                    ...
                                    <select class="form-control select2" name="target_id">
                                        ...
                                        @if(isset($targets) && !empty($targets))
                                            @foreach($targets as $target)
                                                <option {{ old('target_id', $punjabActionPlan->target_id)== $target->id ? 'selected': '' }} value="{{ $target->id }}"> {{ $target->target_value }} </option>
                                            @endforeach
                                        @endif
                                    </select>
                                    @error('target_id')
                                    ...
                                    <select class="form-control select2" name="indicator_id">
                                        ...
                                        @if(isset($indicators) && !empty($indicators))
                                            @foreach($indicators as $indicator)
                                                <option {{ old('indicator_id', $punjabActionPlan->indicator_id)== $indicator->id ? 'selected': '' }} value="{{ $indicator->id }}"> {{ $indicator->indicator_title }} </option>
                                            @endforeach
                                        @endif
                                    </select>
                                    @error('indicator_id')
                            ...
                            <div class="row form-group">
                                ...
                                <div class="col-lg-6">
                                    ...
                                    <input type="text" name="baseline"  class="form-control" placeholder="Baseline" value="{{ old('baseline', $punjabActionPlan->baseline) }}" />
                                    @error('baseline')
                                    <div class="error">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="row form-group">
                                <div class="col-lg-6">
                                    ...
                                    <input type="text" name="reporting_agency"  class="form-control" placeholder="Reporting Agency" value="{{ old('reporting_agency', $punjabActionPlan->reporting_agency) }}" />
                                ...
                                <div class="col-lg-6">
                                    ...
                                    <input type="text" name="implementation_responsibility"  class="form-control" placeholder="Responsibility" value="{{ old('implementation_responsibility', $punjabActionPlan->implementation_responsibility) }}" />
                                ...
                                <div class="row form-group">
                                    <div class="col-lg-12">
                                        <table class="table table-bordered m_repeater_section">
                                            ...
                                            @php
                                                $target_reforms_count= $punjabActionPlan->targetReforms->count() >= count(old('target_reforms',array(1))) ? $punjabActionPlan->targetReforms->count():count(old('target_reforms',array(1)));
                                                $target_reforms = $punjabActionPlan->targetReforms;
                                            @endphp

                                            @for($index =0; $index<$target_reforms_count; $index++)
                                                <tr  data-repeater-item>
                                                    <td>
                                                        <div class="row">
                                                            <div class="col-lg-12">
                                                                <input type="text" name="defining_action" style="width: 100% !important;" class="form-control defining_action" placeholder="Defining action" value="{{ old("target_reforms.{$index}.defining_action", $target_reforms[$index]->defining_action??'') }}" />
                                                    ...
                                                    <td>
                                                        <div class="row">
                                                            <div class="col-lg-12">
                                                                <input readonly type="text" name="defining_date" style="width: 100% !important;" class="form-control defining_date datepicker" placeholder="Defining date" value="{{ old("target_reforms.{$index}.defining_date", $target_reforms[$index]->defining_date??'') }}" />
                                                                ...
                                                                <select class="form-control select2 progress_status" name="progress_status" style="width: 100% !important;">
                                                                    ...
                                                                    @if(isset($progress_status) && !empty($progress_status))
                                                                        @foreach($progress_status as $status)
                                                                            <option {{ old("target_reforms.{$index}.progress_status", $target_reforms[$index]->progress_status??'')== $status ? 'selected': '' }} value="{{ $status }}"> {{ $status }} </option>
                                                                        @endforeach
                                                                    @endif
                                                                </select>
                ...
                document.getElementById('form_add_edit'),
                {
                    fields: {
                        ...
                        indicator_framework_file: {
                            validators: {
                                enabled: {{$punjabActionPlan->indicator_framework_file?'false':'true'}},
                                notEmpty: {
